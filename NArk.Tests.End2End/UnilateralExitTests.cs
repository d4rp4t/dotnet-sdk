...
public class UnilateralExitTests
{
    ...
    private static async Task<ExitTestSetup> FundWalletOffchainForExitAsync()
    {
        ...
        // Wait until the virtual tx branch is populated AND all tree txs carry
        // their MuSig2 key-path signature. The fetch may race the arkd co-sign
        // round, so poll until PSBT_IN_TAP_KEY_SIG is present on every tree input.
        await TestWaiter.WaitFor(async () =>
        {
            ...
            return branch
                .Where(tx => tx.Type == ChainedTxType.Tree && tx.Hex is not null)
                .All(tx =>
                {
                    try
                    {
                        var psbt = PSBT.Parse(tx.Hex!, info.Network);
                        return psbt.Inputs.All(inp => inp.TaprootKeySignature is not null);
                    }
                    catch { return false; }
                });
