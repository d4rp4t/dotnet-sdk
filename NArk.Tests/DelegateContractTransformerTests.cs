...
public class DelegateContractTransformerTests
{
    ...
    private DelegateContractTransformer _transformer = null!;
    ...
    public void SetUp()
    {
        ...
        _transformer = new DelegateContractTransformer(_walletProvider);
    }

    private static ArkDelegateContract CreateDelegateContract() =>
    ...
    {
        var paymentContract = new ArkPaymentContract(TestServerKey, DefaultExitDelay, TestUserKey);
        var vtxo = CreateVtxo();

        var result = await _transformer.CanTransform("wallet-1", paymentContract, vtxo);
    ...
    [Test]
    public async Task CanTransform_ReturnsFalse_WhenNoSigner()
    {
        _walletProvider.GetSignerAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
    ...
    {
        var contract = CreateDelegateContract();
        var delegationTransformer = new DelegateContractDelegationTransformer(_walletProvider);
